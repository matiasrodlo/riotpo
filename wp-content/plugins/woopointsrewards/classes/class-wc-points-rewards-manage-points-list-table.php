...
class WC_Points_Rewards_Manage_Points_List_Table extends WP_List_Table {
		...
		switch ( $column_name ) {
			...
			case 'update':
				$column_content = '<input type="text" class="points_balance" name="points_balance[' . $user_points->user_id . ']" value="' . $user_points->points_balance . '" />' .
					' <a class="button update_points" href="' . add_query_arg( array( 'action' => 'update', 'user_id[]' => $user_points->user_id ) ) . '">' . __( 'Update', 'wc_points_rewards' ) . '</a>';
			break;
	...
	public function process_actions() {
		...
		$user_ids = isset( $_REQUEST['user_id'] ) ? array_map( 'absint', $_REQUEST['user_id'] ): array();
		...
		else $redirect_url = remove_query_arg( array( '_wp_http_referer', '_wpnonce', 'action', 'action2', 'user_id', 'points_balance' ), stripslashes( $_SERVER['REQUEST_URI'] ) );
